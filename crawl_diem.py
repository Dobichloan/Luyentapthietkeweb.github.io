...
def crawl_scores(start_sbd: int, end_sbd: int, year: int, output_file='diem_thi_2025.csv'):
    # Các môn cần lấy điểm
    ...
    with open(output_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        for sbd in range(start_sbd, end_sbd + 1):
            sbd_str = f"{sbd:08d}"  # Format thành chuỗi 8 chữ số
            ...
            try:
                ...
                if data.get("success") and data.get("total", 0) > 0:
                    ...
                    time.sleep(2)  # Tránh gửi quá nhanh

                time.sleep(0.1)  # Tránh gửi quá nhanh

            except Exception as e:
                print(f"⚠️ Error with SBD {sbd_str}: {e}")
                continue

crawl_scores(start_sbd=1000001, end_sbd=1000009, year=2025)